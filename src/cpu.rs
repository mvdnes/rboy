...
impl CPU {
	...
	fn fetchword(&mut self, mmu: &MMU) -> u16 {
	...
	fn call(&mut self, mmu: &mut MMU) -> uint {
		match self.fetchbyte(mmu) {
			...
			0x27 => { self.alu_daa(); 1 },
			0x28 => { if self.reg.getflag(Z) { self.cpu_jr(mmu); 3 } else { self.reg.pc += 1; 2  } },
			...
			0x34 => { let a = self.reg.hl(); let v = mmu.rb(a); mmu.wb(a, self.alu_inc(v)); 3 },
			...
			0x38 => { if self.reg.getflag(C) { self.cpu_jr(mmu); 3 } else { self.reg.pc += 1; 2  } },
			...
			0xA6 => { let v = mmu.rb(self.reg.hl()); self.alu_and(v); 2 },
			...
			0xAE => { let v = mmu.rb(self.reg.hl()); self.alu_xor(v); 2 },
			...
			0xB6 => { let v = mmu.rb(self.reg.hl()); self.alu_or(v); 2 },
			...
			0xBE => { let v = mmu.rb(self.reg.hl()); self.alu_cp(v); 2 },
			0xBF => { self.alu_cp(self.reg.a); 1 },
			0xC0 => { if !self.reg.getflag(Z) { self.reg.pc = self.popstack(mmu); 5 } else { 2 } },
			0xC1 => { let v = self.popstack(mmu); self.reg.setbc(v); 3 },
			0xC2 => { if !self.reg.getflag(Z) { self.reg.pc = self.fetchword(mmu); 4 } else { self.reg.pc += 2; 3 } },
			0xC3 => { self.reg.pc = self.fetchword(mmu); 3 },
			0xC4 => { if !self.reg.getflag(Z) { self.pushstack(mmu, self.reg.pc + 2); self.reg.pc = self.fetchword(mmu); 6 } else { self.reg.pc += 2; 3 } },
			0xC5 => { let v = self.reg.bc(); self.pushstack(mmu, v); 4 },
			0xC6 => { let v = self.fetchbyte(mmu); self.alu_add(v, false); 2 },
			0xC7 => { self.pushstack(mmu, self.reg.pc); self.reg.pc = 0x00; 4 },
			0xC8 => { if self.reg.getflag(Z) { self.reg.pc = self.popstack(mmu); 5 } else { 2 } },
			0xC9 => { self.reg.pc = self.popstack(mmu); 4 },
			0xCA => { if self.reg.getflag(Z) { self.reg.pc = self.fetchword(mmu); 4 } else { self.reg.pc += 2; 3 } },
			0xCB => { self.callCB(mmu) },
			0xCC => { if self.reg.getflag(Z) { self.pushstack(mmu, self.reg.pc + 2); self.reg.pc = self.fetchword(mmu); 6 } else { self.reg.pc += 2; 3 } },
			0xCD => { self.pushstack(mmu, self.reg.pc + 2); self.reg.pc = self.fetchword(mmu); 6 },
			0xCE => { let v = self.fetchbyte(mmu); self.alu_add(v, true); 2 },
			0xCF => { self.pushstack(mmu, self.reg.pc); self.reg.pc = 0x08; 4 },
			0xD0 => { if !self.reg.getflag(C) { self.reg.pc = self.popstack(mmu); 5 } else { 2 } },
			0xD1 => { let v = self.popstack(mmu); self.reg.setde(v); 3 },
			0xD2 => { if !self.reg.getflag(C) { self.reg.pc = self.fetchword(mmu); 4 } else { self.reg.pc += 2; 3 } },
			0xD4 => { if !self.reg.getflag(C) { self.pushstack(mmu, self.reg.pc + 2); self.reg.pc = self.fetchword(mmu); 6 } else { self.reg.pc += 2; 3 } },
			0xD5 => { let v = self.reg.de(); self.pushstack(mmu, v); 4 },
			0xD6 => { let v = self.fetchbyte(mmu); self.alu_sub(v, false); 2 },
			0xD7 => { self.pushstack(mmu, self.reg.pc); self.reg.pc = 0x10; 4 },
			0xD8 => { if self.reg.getflag(C) { self.reg.pc = self.popstack(mmu); 5 } else { 2 } },
			0xD9 => { self.reg.pc = self.popstack(mmu); self.setei = 1; 4 },
			0xDA => { if self.reg.getflag(C) { self.reg.pc = self.fetchword(mmu); 4 } else { self.reg.pc += 2; 3 } },
			0xDC => { if self.reg.getflag(C) { self.pushstack(mmu, self.reg.pc + 2); self.reg.pc = self.fetchword(mmu); 6 } else { self.reg.pc += 2; 3 } },
			0xDE => { let v = self.fetchbyte(mmu); self.alu_sub(v, true); 2 },
			0xDF => { self.pushstack(mmu, self.reg.pc); self.reg.pc = 0x18; 4 },
			...
			0xF1 => { let v = self.popstack(mmu) & 0xFFF0; self.reg.setaf(v); 3 },
			0xF2 => { self.reg.a = mmu.rb(0xFF00 + self.reg.c as u16); 2 },
			0xF3 => { self.setdi = 2; 1 },
			0xF5 => { let v = self.reg.af(); self.pushstack(mmu, v); 4 },
			0xF6 => { let v = self.fetchbyte(mmu); self.alu_or(v); 2 },
	...
	fn callCB(&mut self, mmu: &mut MMU) -> uint {
		match self.fetchbyte(mmu) {
			...
			0x06 => { let a = self.reg.hl(); let v = mmu.rb(a); mmu.wb(a, self.alu_rlc(v)); 4 },
			...
			0x0E => { let a = self.reg.hl(); let v = mmu.rb(a); mmu.wb(a, self.alu_rrc(v)); 4 },
			...
			0x16 => { let a = self.reg.hl(); let v = mmu.rb(a); mmu.wb(a, self.alu_rl(v)); 4 },
			...
			0x1E => { let a = self.reg.hl(); let v = mmu.rb(a); mmu.wb(a, self.alu_rr(v)); 4 },
			...
			0x26 => { let a = self.reg.hl(); let v = mmu.rb(a); mmu.wb(a, self.alu_sla(v)); 4 },
			...
			0x2E => { let a = self.reg.hl(); let v = mmu.rb(a); mmu.wb(a, self.alu_sra(v)); 4 },
			...
			0x36 => { let a = self.reg.hl(); let v = mmu.rb(a); mmu.wb(a, self.alu_swap(v)); 4 },
			...
			0x3E => { let a = self.reg.hl(); let v = mmu.rb(a); mmu.wb(a, self.alu_srl(v)); 4 },
			...
			0x46 => { let a = self.reg.hl(); self.alu_bit(mmu.rb(a), 0); 4 },
			...
			0x4E => { let a = self.reg.hl(); self.alu_bit(mmu.rb(a), 1); 4 },
			...
			0x56 => { let a = self.reg.hl(); self.alu_bit(mmu.rb(a), 2); 4 },
			...
			0x5E => { let a = self.reg.hl(); self.alu_bit(mmu.rb(a), 3); 4 },
			...
			0x66 => { let a = self.reg.hl(); self.alu_bit(mmu.rb(a), 4); 4 },
			...
			0x6E => { let a = self.reg.hl(); self.alu_bit(mmu.rb(a), 5); 4 },
			...
			0x76 => { let a = self.reg.hl(); self.alu_bit(mmu.rb(a), 6); 4 },
			...
			0x7E => { let a = self.reg.hl(); self.alu_bit(mmu.rb(a), 7); 4 },
			...
			0x86 => { let a = self.reg.hl(); let v = mmu.rb(a) & !(1 << 0); mmu.wb(a, v); 4 },
			...
			0x8E => { let a = self.reg.hl(); let v = mmu.rb(a) & !(1 << 1); mmu.wb(a, v); 4 },
			...
			0x96 => { let a = self.reg.hl(); let v = mmu.rb(a) & !(1 << 2); mmu.wb(a, v); 4 },
			...
			0x9E => { let a = self.reg.hl(); let v = mmu.rb(a) & !(1 << 3); mmu.wb(a, v); 4 },
			...
			0xA6 => { let a = self.reg.hl(); let v = mmu.rb(a) & !(1 << 4); mmu.wb(a, v); 4 },
			...
			0xAE => { let a = self.reg.hl(); let v = mmu.rb(a) & !(1 << 5); mmu.wb(a, v); 4 },
			...
			0xB6 => { let a = self.reg.hl(); let v = mmu.rb(a) & !(1 << 6); mmu.wb(a, v); 4 },
			...
			0xBE => { let a = self.reg.hl(); let v = mmu.rb(a) & !(1 << 7); mmu.wb(a, v); 4 },
			...
			0xC6 => { let a = self.reg.hl(); let v = mmu.rb(a) | (1 << 0); mmu.wb(a, v); 4 },
			...
			0xCE => { let a = self.reg.hl(); let v = mmu.rb(a) | (1 << 1); mmu.wb(a, v); 4 },
			...
			0xD6 => { let a = self.reg.hl(); let v = mmu.rb(a) | (1 << 2); mmu.wb(a, v); 4 },
			...
			0xDE => { let a = self.reg.hl(); let v = mmu.rb(a) | (1 << 3); mmu.wb(a, v); 4 },
			...
			0xE6 => { let a = self.reg.hl(); let v = mmu.rb(a) | (1 << 4); mmu.wb(a, v); 4 },
			...
			0xEE => { let a = self.reg.hl(); let v = mmu.rb(a) | (1 << 5); mmu.wb(a, v); 4 },
			...
			0xF6 => { let a = self.reg.hl(); let v = mmu.rb(a) | (1 << 6); mmu.wb(a, v); 4 },
			...
			0xFE => { let a = self.reg.hl(); let v = mmu.rb(a) | (1 << 7); mmu.wb(a, v); 4 },
	...
	fn alu_add16imm(&mut self, mmu: &MMU, a: u16) -> u16 {
		let b = self.fetchbyte(mmu) as i8 as i16 as u16;
