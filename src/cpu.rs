...
impl CPU {
	...
	fn call(&mut self, mmu: &mut MMU) -> uint {
		match self.fetchbyte(mmu) {
			...
			0xC3 => { self.reg.pc = self.fetchword(mmu); 4 },
	...
	fn callCB(&mut self, mmu: &mut MMU) -> uint {
		match self.fetchbyte(mmu) {
			...
			0x46 => { let a = self.reg.hl(); self.alu_bit(mmu.rb(a), 0); 3 },
			...
			0x4E => { let a = self.reg.hl(); self.alu_bit(mmu.rb(a), 1); 3 },
			...
			0x56 => { let a = self.reg.hl(); self.alu_bit(mmu.rb(a), 2); 3 },
			...
			0x5E => { let a = self.reg.hl(); self.alu_bit(mmu.rb(a), 3); 3 },
			...
			0x66 => { let a = self.reg.hl(); self.alu_bit(mmu.rb(a), 4); 3 },
			...
			0x6E => { let a = self.reg.hl(); self.alu_bit(mmu.rb(a), 5); 3 },
			...
			0x76 => { let a = self.reg.hl(); self.alu_bit(mmu.rb(a), 6); 3 },
			...
			0x7E => { let a = self.reg.hl(); self.alu_bit(mmu.rb(a), 7); 3 },
